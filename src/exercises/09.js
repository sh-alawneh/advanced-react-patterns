...
class Toggle extends React.Component {
  ...
  internalSetState(changes, callback) {
    this.setState(state => {
      ...
      // `{type, ...c}`
      const {type, ...userChanges} = reducedChanges;
      return Object.keys(userChanges).length
        ? userChanges
        : null
    }, callback)
  }
  reset = () =>
    ...
    this.internalSetState({...this.initialState, type: Toggle.changesTypes.reset}, () =>
      this.props.onReset(this.state.on),
    )
  // 🐨 accept a `type` property here and give it a default value
  toggle = ({type = Toggle.changesTypes.toggle} = {}) =>
