...
const ToggleConsumer = (props) => {
  return (
    <ToggleContext.Consumer>
      {context => context ?  props.children(context) : Error("Either No Value Provided To The Provider Or The Context Does Not Have A Default Values")}
    </ToggleContext.Consumer>  
  )
}
...
class Toggle extends React.Component {
  ...
  static Off = ({children}) => <ToggleConsumer>{context => context.on ? null : children}</ToggleConsumer>
  static Button = ({...props}) => (
    <ToggleConsumer>
     {context => <Switch on={context.on} onClick={context.toggle} {...props} />}
    </ToggleConsumer>
  )
